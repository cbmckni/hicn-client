...
# Masquerade all the traffic coming from VPP
iptables -t nat -A POSTROUTING -j MASQUERADE \
  --src ${TAP_ADDRESS_NET} ! --dst ${TAP_ADDRESS_NET} -o ${NETWORK_INTERFACE}
...
vppctl hicn punting add prefix ${FIRST_IPV6_WORD}::/16 \
  intfc ${TAP_NAME} type udp4 dst_port ${HICN_LISTENER_PORT}
...
iptables -t nat -A POSTROUTING --out-interface ${NETWORK_DEVICE} -j MASQUERADE
